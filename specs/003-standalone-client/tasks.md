...
- [x] T001 Add `dist/`, `build/risus/`, and `risus.cfg` entries to
  `.gitignore`
- [x] T002 [P] Create `build/` directory with placeholder `build/README.md`
  (full content added in T013)
- [x] T003 [P] Create `risus.cfg.example` at project root per
  `specs/003-standalone-client/contracts/config-file.md` — include
  commented `server` and `name` keys with auto-save note
- [x] T004 Add `pyinstaller` to `[project.optional-dependencies]` `dev`
...
- [x] T005 Implement `client/config.py` with two public functions:
...
- [x] T006 [P] Unit tests for `client/config.py` in
...
- [x] T007 [US1] Update `risus.py` — replace positional `sys.argv` access
...
- [x] T008 [P] [US1] Unit tests for startup prompt logic in
  ...
  cases.
- [x] T009 [US1] Create `build/risus.spec` — PyInstaller spec file with:
...
- [x] T010 [US1] Improve server-unreachable error message in `risus.py` —
...
- [x] T011 [US2] Create `.github/workflows/build.yml` — on push/PR to
...
- [x] T012 [US2] Create `.github/workflows/release.yml` — trigger on `push`
...
- [x] T013 [US2] Write complete developer build instructions in
...
- [x] T014 [US3] Create `PLAYER.md` at project root — adapt content from
  ...
  from) the release.
- [x] T015 [P] [US3] Update project root `README.md` — add a "Playing the
...
- [x] T016 [P] Verify `risus.cfg` and `dist/` absent from `git status`
  after a local build (gitignore correct per T001)
- [x] T017 Run `pytest tests/unit -q` and confirm all new tests pass
  (T006, T008)
- [x] T018 Run `CONTAINER_ENGINE=podman pytest tests/e2e -m e2e -q` against
...
- [x] T019 Run end-to-end smoke test: build binary, copy to temp dir (no
  Python in PATH), run with valid server args, confirm no traceback on clean
  exit and `risus.cfg` written next to binary
- [x] T020 [P] Run `ruff check` on `risus.py` and `client/config.py`; fix
  any lint errors
