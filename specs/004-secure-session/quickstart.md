...
```bash
python risus.py
# Server address [saved]: risus.boos.systems
...
```bash
python risus.py risus.boos.systems Conan --token my-super-secret-game-token
```
...
CONTAINER_ENGINE=podman PATH=$PWD/.venv/bin:$PATH RISUS_TOKEN=testtoken pytest tests/e2e -m e2e -q
```
